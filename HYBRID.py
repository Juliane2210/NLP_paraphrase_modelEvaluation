...
def load_embeddings_from_pkl(file_path):
...
def compute_cosine_similarity_test():
    ...
    #
    # In the hybrid, use the embeddings from SBERT
    #
    embedding1, embedding2 = load_embeddings_from_pkl('SBERT_embeddings.pkl')
    ...
    reshaped = np.array(similarity_scores_for_pairs).reshape(-1, 1)
    return reshaped
...
def preprocess_test(df):
    cosine_similarities = compute_cosine_similarity_test()
...
if __name__ == "__main__":
    ...
    sentences_test, labels_test = preprocess_test(testDataset)
    ...
    print("----- LOGISTIC REGRESSION HYBRID Training Model -----")
    model.fit(X_train, y_train)

    print("-----  LOGISTIC REGRESSION HYBRID Model Evaluation -----")
    evaluate_model(model, X_test, y_test)

    print("----- LOGISTIC REGRESSION HYBRID Fine-tuning Model -----")
    tuned_model = fine_tune_model(X_train, y_train)
    print("----- LOGISTIC REGRESSION HYBRID Tuned Model Evaluation -----")
